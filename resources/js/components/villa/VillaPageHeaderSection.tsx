...
export default function VillaPageHeaderSection({
    ...
    const meta: Array<{ icon: React.ElementType; label: string; show: boolean }> = [
        { icon: Users, label: `Tamu maks ${villa.max_guests}`, show: (villa.max_guests ?? 0) > 0 },
        { icon: Bed, label: `${villa.bedrooms} kamar tidur`, show: (villa.bedrooms ?? 0) > 0 },
        { icon: Bed, label: `${villa.beds} tempat tidur`, show: (villa.beds ?? 0) > 0 },
        { icon: Bath, label: `${villa.bathrooms} kamar mandi`, show: (villa.bathrooms ?? 0) > 0 },
    ].filter(m => m.show);
